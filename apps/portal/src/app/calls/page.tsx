...
import { Phone, PhoneIncoming, PhoneOutgoing, Clock, CheckCircle2, AlertCircle, ArrowLeftRight } from 'lucide-react'
...
async function fetchCalls(page = 1): Promise<ListResponse> {
  try {
    const res = await fetch(
      `${process.env.API_INTERNAL_URL ?? 'http://localhost:3001'}/api/v1/calls?page=${page}&limit=20`,
      { cache: 'no-store' },
    )
...
export default async function CallsPage() {
  const { data: calls, total } = await fetchCalls()
      ...
      {calls.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-[var(--color-text-muted)]">
            Cuộc gọi sẽ xuất hiện ở đây sau khi Voice Worker được kết nối.
          </p>
        </div>
      ) : (
