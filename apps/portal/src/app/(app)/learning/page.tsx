...
async function fetchProposals(status = 'pending'): Promise<Proposal[]> {
  try {
    return await serverFetch<Proposal[]>(`/learning/proposals?status=${status}`)
  } catch {
    return []
  }
}
...
export default async function LearningPage({
  ...
  const activeStatus = (params.status ?? 'pending') as ProposalStatus
  const proposals = await fetchProposals(activeStatus)
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--color-text-muted)] mt-1">
          HITL review — {proposals.length} đề xuất
        </p>
      </div>
      ...
      {proposals.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h3 className="text-base font-semibold text-[var(--color-text)] mb-1">
            Không có đề xuất {STATUS_META[activeStatus].label.toLowerCase()}
          </h3>
